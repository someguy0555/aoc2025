...
main = do
  ...
  print . product . take 3 . reverse . sort . map length $ getCircuits points (take 1000 graph)

getCircuits :: [Point] -> Graph -> [Circuit]
getCircuits points graph = getCircuits' graph $ map (: []) points

getCircuits' :: Graph -> [Circuit] -> [Circuit]
getCircuits' [] currCircuits = currCircuits
getCircuits' ((p1, _, p2) : connLeft) currCircuits = getCircuits' connLeft . connectCircuits p1 p2 $ currCircuits
